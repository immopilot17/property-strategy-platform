...
export abstract class OfficialWebProvider implements FundingProvider {
  ...
  async discover() {
    ...
    for (const catalogUrl of this.catalogUrls) {
      const response = await fetch(catalogUrl, { headers: { "User-Agent": "PropertyStrategyFundingBot/1.0" }, cache: "no-store" });
      if (!response.ok) throw new Error(`${this.name}: Katalog nicht erreichbar (${response.status}).`);
      const $ = cheerio.load(await response.text());
      $("a[href]").each((_, element) => {
        try {
          const url = new URL($(element).attr("href")!, catalogUrl);
          url.search = ""; url.hash = "";
  ...
  async collect(sourceUrl: string): Promise<FundingSourceDocument> {
    ...
    const response = await fetch(url, { headers: { "User-Agent": "PropertyStrategyFundingBot/1.0" }, cache: "no-store" });
